package com.Cinema_Management_System.Inform.handler;
...
//        if (userSocketSessionMap.get(uid) == null) {
        ...
        if ("customer".equals(identity)) {
            ...
            List<Inform> messages = infoService.selectAllInfo(Integer.parseInt(uid));
            ...
            List<Inform> messages = infoService.selectInfoByType();
...
//        }
    ...
    public void handleMessage(WebSocketSession webSocketSession, WebSocketMessage<?> webSocketMessage) throws Exception {
        ...
        Inform msg = new Gson().fromJson(webSocketMessage.getPayload().toString(), Inform.class);
...
//            msg.setInfoId(id);
            ...
            sendMessageToUser(String.valueOf(msg.getCustomerId()), "manager", "boss", new TextMessage(new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss").create().toJson(msg)));
        } else {
            msg.setInfoStatus("未读");
...
//            msg.setInfoId(id);
            System.out.println("发送的消息是" + msg);
            sendMessageToUser(identity, String.valueOf(msg.getCustomerId()), "", new TextMessage(new GsonBuilder().setDateFormat("yyyy-MM-dd HH:mm:ss").create().toJson(msg)));
        }
...
// */
    ...
    //发送信息的实现
    public void sendMessageToUser(String from, String to, String standby, TextMessage message)
            throws IOException {
        System.out.println("检查是否在线");
        WebSocketSession sessionTo = userSocketSessionMap.get(to);
        WebSocketSession sessionFrom = userSocketSessionMap.get(from);
        WebSocketSession sessionStandby = userSocketSessionMap.get(standby);
//        System.out.println(session);
//        System.out.println(session.isOpen());
        if (sessionTo != null && sessionTo.isOpen()) {
            System.out.println("在线--发送信息---" + message);
            sessionTo.sendMessage(message);
        } else if (sessionStandby != null && sessionStandby.isOpen()) {
            System.out.println("在线--发送信息---" + message);
            sessionStandby.sendMessage(message);

        } else {
            System.out.println("不在线---并返回数据");
        }
        sessionFrom.sendMessage(message);
    }
